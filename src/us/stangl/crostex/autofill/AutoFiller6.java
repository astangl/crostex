...
//System.out.println("Couldn't get nextBestChoice for " + cheapestWord + ", adding to wordThatNeedChange and backtracking");
				...
				List<GridWord> wordsThatNeedChange = new ArrayList<GridWord>();
				wordsThatNeedChange.add(cheapestWord);
...
//					GridWord prevWord = workTuple.word_;
					...
					if (workTuple.getNextBestChoice(wordsThatNeedChange)) {
						workStack.push(workTuple);
						continue MAINLOOP;
					}
				}
	...
	private static class WorkTuple {
		 ...
		 * @param wordsThatNeedChange words that need change, already popped off the stack
		 * @return true if able to find another possibility for this tuple, intersecting one of the specified wordsThatNeedChange
		 */
		public boolean getNextBestChoice(List<GridWord> wordsThatNeedChange) {
			...
			for (GridWord needToChangeWord : wordsThatNeedChange) {
				if (needToChangeWord.getDirection() == wordDirection)
					continue;												// cannot intersect with parallel word, so don't even bother checking
				
				Cell intersection = word_.getIntersection(needToChangeWord);
				if (intersection != null)
					intersectionCells.add(intersection);
			}
			
			if (intersectionCells.isEmpty()) {
				// Restore this word
			...
			if (getNextBestChoiceImpl(intersectionCells) || getBestChoiceOrRollback())
				return true;
			
			// Add our word to the words that need to change for further rollback
			wordsThatNeedChange.add(word_);
			return false;
		}

		private boolean getNextBestChoiceImpl(List<Cell> avoidCells) {
			char[] cellContents = new char[avoidCells.size()];
			int[] patternIndex = new int[avoidCells.size()];
			for (int i = 0; i < avoidCells.size(); ++i) {
				Cell avoidCell = avoidCells.get(i);
				cellContents[i] = avoidCell.getContents().charAt(0);
				patternIndex[i] = word_.indexOfCellInPattern(avoidCell);
			}
...
//				if (avoidCell != null && avoidCell.getContents().equals(cellContents))
				if (avoidCells.size() < 3) {
					boolean somethingDifferent = false;
					for (int i = 0; i < cellContents.length; ++i) {
						if (choiceWord[patternIndex[i]] != cellContents[i]) {
